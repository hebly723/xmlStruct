...
public class AlterScoreActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState){
        ...
        getSupportActionBar().setTitle("成绩修改");
        ...
        toastYes = Toast.makeText(this, "修改成功",Toast.LENGTH_LONG);
        toastNo = Toast.makeText(this, "修改失败",Toast.LENGTH_LONG);
        ...
        int number = getIntent().getExtras().getInt("position");
        Log.v("Tag", number+"&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&");
        CustomUser cu =  jsonArray.get(number);

        Log.v("Tag", cu.toString()+"&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&");
        studentNumber.setText(cu.getId());
        ((RadioButton)sex.getChildAt(cu.getSex())).setChecked(true);
        studentNumber.setEnabled(false);
        text_grade.setText(cu.getGrade()+"");
        studentName.setText(cu.getUsername());
        int i = 0, signal = 0;
        for ( String string : arrayList)
        {
            if (string.trim().equals(cu.getAddress().trim()))
                signal = i;
            i ++;
        }
        address.setSelection(signal , true);
//        sex.check(R.id.alter_woman);
//        address.setId();
//        sex.check(((RadioButton)findViewById(R.id.alter_woman)).getId());
//        CheckDlgItem( R.id.alter_woman,BST_CHECKED);
//        int sexOption = 0;
//        if (cu.getSex() == null)
//            sex.check(R.id.alter_man);
//        else if (cu.getSex() > 0)
//            sex.check(R.id.alter_woman);
//        else
//            sex.check(R.id.alter_man);
...
//                HttpRequest.postDownloadJson("http://192.168.1.233:8080/Grade/login.action", user.toString2());
                    try {
                        ...
                        realUrl = new URL("http://" + settings.getString("service_ip", "").trim() + ":" +
                                settings.getString("service_port", "").trim() + "/Grade/alterGrade.action");
...
//                        Log.v("Tag", "address position"+address.getSelectedItemPosition()+"");
                        ...
                        if (radio.getText().toString().trim().equals("男"))
                            ...
                            cuser.setSex(1);
                        Log.v("Tag", "6");
                        Log.v("Tag", R.id.alter_woman+"");
//                        if (((RadioButton)findViewById(R.id.alter_woman)).isChecked())
//                            cuser.setSex(1);
//                        else
//                            cuser.setSex(0);
                        Log.v("Tag", "6");
